...
export class Visual implements IVisual {
            ...
            <div id="musicStudio" style="display: flex; flex-direction: column; height: 100%; background: ${this.themes.get(this.currentTheme)?.colors.background}; color: #fff; position: relative; overflow: hidden;">
                ...
                <div id="audioControls" style="position: relative; z-index: 10; padding: 8px; background: rgba(0,0,0,0.8); border-radius: 8px; margin: 8px; box-shadow: 0 4px 16px rgba(0,255,136,0.4); backdrop-filter: blur(10px);">
                    ...
                    <div style="margin-bottom: 4px;">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 2px; font-size: 9px; color: #aaa;">
                            <span id="currentTime">0:00</span>
                            <span id="trackMetadata" style="color: ${this.themes.get(this.currentTheme)?.colors.primary}; font-weight: bold; font-size: 9px;"></span>
                            <span id="totalTime">0:00</span>
                        </div>
                        <div style="background: #333; height: 4px; border-radius: 2px; position: relative; cursor: pointer; border: 1px solid #555;" id="progressContainer">
                            <div id="progressBar" style="background: linear-gradient(90deg, ${this.themes.get(this.currentTheme)?.colors.primary}, ${this.themes.get(this.currentTheme)?.colors.secondary}); height: 100%; width: 0%; border-radius: 2px; transition: width 0.1s;"></div>
                        </div>
                    </div>
                    
                    <!-- Audio Effects Controls -->
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(60px, 1fr)); gap: 4px; margin-bottom: 6px; padding: 4px; background: rgba(0,0,0,0.3); border-radius: 4px;">
                        <label style="display: flex; flex-direction: column; align-items: center; font-size: 8px; gap: 1px;">
                            🔊 Vol
                            <input type="range" id="volumeSlider" min="0" max="100" value="70" style="width: 100%; accent-color: ${this.themes.get(this.currentTheme)?.colors.primary}; height: 3px;">
                            <span id="volumeDisplay" style="color: ${this.themes.get(this.currentTheme)?.colors.primary}; font-weight: bold; font-size: 7px;">70%</span>
                        </label>
                        <label style="display: flex; flex-direction: column; align-items: center; font-size: 8px; gap: 1px;">
                            🎸 Bass
                            <input type="range" id="bassSlider" min="-12" max="12" value="0" style="width: 100%; accent-color: #ff6b6b; height: 3px;">
                            <span id="bassDisplay" style="color: #ff6b6b; font-weight: bold; font-size: 7px;">0dB</span>
                        </label>
                        <label style="display: flex; flex-direction: column; align-items: center; font-size: 8px; gap: 1px;">
                            🎺 Treb  
                            <input type="range" id="trebleSlider" min="-12" max="12" value="0" style="width: 100%; accent-color: #4ecdc4; height: 3px;">
                            <span id="trebleDisplay" style="color: #4ecdc4; font-weight: bold; font-size: 7px;">0dB</span>
                        </label>
                        <label style="display: flex; flex-direction: column; align-items: center; font-size: 8px; gap: 1px;">
                            🌊 Rev
                            <input type="range" id="reverbSlider" min="0" max="100" value="0" style="width: 100%; accent-color: #a8e6cf; height: 3px;">
                            <span id="reverbDisplay" style="color: #a8e6cf; font-weight: bold; font-size: 7px;">0%</span>
                        </label>
                    </div>
                </div>
                
                <!-- DJ Mashup Mode -->
                <div id="djMashupPanel" style="display: none; margin: 4px; padding: 8px; background: rgba(20,20,40,0.95); border-radius: 6px; border: 2px solid #ff6b35; box-shadow: 0 4px 16px rgba(255,107,53,0.4);">
                    <div style="text-align: center; margin-bottom: 6px; color: #ff6b35; font-weight: bold; font-size: 12px;">
                        🎧 DJ MASHUP MODE 🎧
                    </div>
                    
                    <!-- Track Selection -->
                    <div style="display: grid; grid-template-columns: 1fr 50px 1fr; gap: 8px; margin-bottom: 8px; align-items: center;">
                        <!-- Track A -->
                        <div style="background: rgba(255,0,100,0.2); padding: 6px; border-radius: 4px; border: 1px solid #ff0064;">
                            <div style="text-align: center; font-weight: bold; color: #ff0064; margin-bottom: 4px; font-size: 11px;">TRACK A</div>
                            <input type="file" id="trackAInput" accept=".mp3,.wav,.ogg,.m4a" 
                                   style="width: 100%; padding: 2px; background: #333; color: #fff; border: 1px solid #ff0064; border-radius: 3px; font-size: 9px; margin-bottom: 3px;">
                            <div style="display: flex; gap: 2px; align-items: center; font-size: 9px; margin-bottom: 2px;">
                                <span>BPM:</span>
                                <div style="display: flex; align-items: center; gap: 1px;">
                                    <button id="bpmDownA" style="padding: 1px 2px; background: #ff0064; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold; line-height: 1;">
                                        −
                                    </button>
                                    <input type="number" id="bpmA" value="120" min="60" max="200" 
                                           style="width: 35px; padding: 1px; background: #333; color: #fff; border: 1px solid #ff0064; border-radius: 2px; font-size: 9px; text-align: center;">
                                    <button id="bpmUpA" style="padding: 1px 2px; background: #ff0064; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold; line-height: 1;">
                                        +
                                    </button>
                                </div>
                            </div>
                            <div style="display: flex; gap: 2px; align-items: center; font-size: 9px; margin-bottom: 3px;">
                                <span>Vol:</span>
                                <input type="range" id="volumeA" min="0" max="100" value="80" 
                                       style="flex: 1; accent-color: #ff0064; height: 3px;">
                                <span id="volumeADisplay" style="color: #ff0064; font-weight: bold; font-size: 8px;">80</span>
                            </div>
                            <div style="display: flex; gap: 1px; align-items: center; margin-bottom: 2px; font-size: 8px;">
                                <button id="nudgeSlowA" style="padding: 1px 3px; background: #ff0064; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold;">◀</button>
                                <button id="nudgeFastA" style="padding: 1px 3px; background: #ff0064; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold;">▶</button>
                                <button id="tapBeatA" style="padding: 1px 3px; background: #ff6b35; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold;">TAP</button>
                            </div>
                            <div id="trackAInfo" style="color: #ccc; font-size: 8px; text-align: center; min-height: 10px; margin-bottom: 2px;">
                                No track loaded
                            </div>
                            <!-- Track A Position Slider -->
                            <div>
                                <div style="display: flex; justify-content: space-between; margin-bottom: 1px; font-size: 7px; color: #aaa;">
                                    <span id="currentTimeA">0:00</span>
                                    <span id="totalTimeA">0:00</span>
                                </div>
                                <div style="display: flex; align-items: center; gap: 2px;">
                                    <input type="range" id="positionSliderA" min="0" max="100" value="0" 
                                           style="flex: 1; accent-color: #ff0064; height: 2px;" disabled>
                                    <button id="playPauseA" style="padding: 1px 4px; background: #ff0064; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 8px; font-weight: bold;" disabled>
                                        ▶️
                                    </button>
                                </div>
                            </div>
                        ...
                        <div style="text-align: center;">
                            <div style="color: #fff; font-weight: bold; margin-bottom: 2px; font-size: 8px;">CROSS</div>
                            <input type="range" id="crossfader" min="-100" max="100" value="0" 
                                   style="width: 40px; transform: rotate(-90deg); accent-color: #ffff00; height: 4px;">
                            <div id="crossfaderDisplay" style="color: #ffff00; font-weight: bold; font-size: 7px; margin-top: 2px;">CENTER</div>
                        </div>
                        
                        <!-- Track B -->
                        <div style="background: rgba(0,100,255,0.2); padding: 6px; border-radius: 4px; border: 1px solid #0064ff;">
                            <div style="text-align: center; font-weight: bold; color: #0064ff; margin-bottom: 4px; font-size: 11px;">TRACK B</div>
                            <input type="file" id="trackBInput" accept=".mp3,.wav,.ogg,.m4a" 
                                   style="width: 100%; padding: 2px; background: #333; color: #fff; border: 1px solid #0064ff; border-radius: 3px; font-size: 9px; margin-bottom: 3px;">
                            <div style="display: flex; gap: 2px; align-items: center; font-size: 9px; margin-bottom: 2px;">
                                <span>BPM:</span>
                                <div style="display: flex; align-items: center; gap: 1px;">
                                    <button id="bpmDownB" style="padding: 1px 2px; background: #0064ff; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold; line-height: 1;">
                                        −
                                    </button>
                                    <input type="number" id="bpmB" value="120" min="60" max="200" 
                                           style="width: 35px; padding: 1px; background: #333; color: #fff; border: 1px solid #0064ff; border-radius: 2px; font-size: 9px; text-align: center;">
                                    <button id="bpmUpB" style="padding: 1px 2px; background: #0064ff; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold; line-height: 1;">
                                        +
                                    </button>
                                </div>
                            </div>
                            <div style="display: flex; gap: 2px; align-items: center; font-size: 9px; margin-bottom: 3px;">
                                <span>Vol:</span>
                                <input type="range" id="volumeB" min="0" max="100" value="80" 
                                       style="flex: 1; accent-color: #0064ff; height: 3px;">
                                <span id="volumeBDisplay" style="color: #0064ff; font-weight: bold; font-size: 8px;">80</span>
                            </div>
                            <div style="display: flex; gap: 1px; align-items: center; margin-bottom: 2px; font-size: 8px;">
                                <button id="nudgeSlowB" style="padding: 1px 3px; background: #0064ff; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold;">◀</button>
                                <button id="nudgeFastB" style="padding: 1px 3px; background: #0064ff; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold;">▶</button>
                                <button id="tapBeatB" style="padding: 1px 3px; background: #ff6b35; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 7px; font-weight: bold;">TAP</button>
                            </div>
                            <div id="trackBInfo" style="color: #ccc; font-size: 8px; text-align: center; min-height: 10px; margin-bottom: 2px;">
                                No track loaded
                            </div>
                            <!-- Track B Position Slider -->
                            <div>
                                <div style="display: flex; justify-content: space-between; margin-bottom: 1px; font-size: 7px; color: #aaa;">
                                ...
                                <div style="display: flex; align-items: center; gap: 2px;">
                                    <input type="range" id="positionSliderB" min="0" max="100" value="0" 
                                           style="flex: 1; accent-color: #0064ff; height: 2px;" disabled>
                                    <button id="playPauseB" style="padding: 1px 4px; background: #0064ff; color: #000; border: none; border-radius: 2px; cursor: pointer; font-size: 8px; font-weight: bold;" disabled>
                                        ▶️
                                    </button>
                                </div>
                            </div>
                    ...
                    <div style="display: flex; justify-content: center; gap: 4px; margin-bottom: 4px;">
                        <button id="syncTracksBtn" style="padding: 4px 8px; background: #00ff88; color: #000; border: none; border-radius: 3px; cursor: pointer; font-weight: bold; font-size: 9px;">
                            🎯 SYNC
                        </button>
                        <button id="mashupPlayBtn" style="padding: 4px 8px; background: #ff6b35; color: #000; border: none; border-radius: 3px; cursor: pointer; font-weight: bold; font-size: 9px;">
                            ▶️ MASHUP
                        </button>
                        <button id="autoMixBtn" style="padding: 4px 8px; background: #8a2be2; color: #fff; border: none; border-radius: 3px; cursor: pointer; font-weight: bold; font-size: 9px;">
                            🎪 AUTO
                        </button>
                    </div>
                    
                    <!-- Sync Info -->
                    <div style="text-align: center; font-size: 8px; color: #ccc;">
                        <span id="syncStatus">🔴 Not synced</span>
                    </div>
                </div>
                
                <!-- Toggle DJ Mashup Mode Button -->
                <div style="text-align: center; margin: 4px;">
                    <button id="toggleDjMode" style="padding: 4px 8px; background: linear-gradient(45deg, #ff6b35, #f7931e); color: #000; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; font-size: 10px; box-shadow: 0 2px 6px rgba(255,107,53,0.4);">
                        🎧 DJ MODE 🎧
                    </button>
                </div>
                
                <!-- Master BPM Control -->
                <div id="masterBpmControl" style="margin: 4px; padding: 6px; background: linear-gradient(135deg, #2d2d2d, #1a1a1a); border-radius: 6px; text-align: center; border: 1px solid #ffaa00; display: none; box-shadow: 0 2px 8px rgba(255,170,0,0.3); position: relative;">
                    <!-- Beat Flash Indicator -->
                    <div id="beatFlash" style="position: absolute; top: -2px; left: -2px; right: -2px; height: 4px; background: rgba(255,255,255,0.8); border-radius: 6px 6px 0 0; opacity: 0; transition: opacity 0.1s; box-shadow: 0 0 10px rgba(255,255,255,0.5);"></div>
                    <div style="color: #ffff00; font-weight: bold; margin-bottom: 4px; text-shadow: 0 0 8px #ffff00; font-size: 10px;">🎵 MASTER BPM 🎵</div>
                    <div style="display: flex; align-items: center; justify-content: center; gap: 6px;">
                        <button id="masterBpmDown" style="width: 24px; height: 24px; background: linear-gradient(45deg, #ff6600, #ffaa00); color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; font-size: 12px; box-shadow: 0 1px 4px rgba(255,102,0,0.4);">−</button>
                        <div style="display: flex; align-items: center; gap: 4px; background: rgba(255,255,0,0.1); padding: 3px 6px; border-radius: 4px; border: 1px solid #ffaa00;">
                            <span style="color: #ffff00; font-weight: bold; font-size: 8px;">BPM:</span>
                            <input type="number" id="masterBpm" min="60" max="200" value="120" style="width: 40px; padding: 2px; text-align: center; border-radius: 3px; border: 1px solid #ffaa00; background: #333; color: #ffff00; font-weight: bold; font-size: 10px;">
                        </div>
                        <button id="masterBpmUp" style="width: 24px; height: 24px; background: linear-gradient(45deg, #ff6600, #ffaa00); color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; font-size: 12px; box-shadow: 0 1px 4px rgba(255,102,0,0.4);">+</button>
                        <button id="syncTracksToMaster" style="padding: 3px 6px; background: linear-gradient(45deg, #00ff00, #00aa00); color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; font-size: 8px; box-shadow: 0 1px 4px rgba(0,255,0,0.4);">SYNC</button>
                        <button id="toggleAutoSync" style="padding: 3px 6px; background: linear-gradient(45deg, #0099ff, #0066cc); color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; font-size: 8px; box-shadow: 0 1px 4px rgba(0,153,255,0.4);">AUTO</button>
                    </div>
                    <div id="masterSyncStatus" style="margin-top: 2px; font-size: 7px; color: #aaa; font-style: italic;">Master control ready</div>
                </div>
                
                <!-- Visualizer Canvas Stack -->
                <div class="canvas-container" style="flex: 1; padding: 0 4px 4px 4px; position: relative; z-index: 5; min-height: 300px; max-height: calc(100vh - 60px);">
                ...
                <div id="shortcutsHelp" style="position: absolute; bottom: 10px; right: 10px; background: rgba(0,0,0,0.9); padding: 12px; border-radius: 8px; font-size: 11px; color: #aaa; display: none; z-index: 15; border: 1px solid #444;">
                    ...
                    <div style="color: ${this.themes.get(this.currentTheme)?.colors.accent}; margin-top: 3px;">🎵 Music Generation: M: Toggle | G: Change Scale</div>
                    <div style="color: #ff6b35; margin-top: 2px;">🎧 DJ Mashup Mode: J: Toggle DJ Mode</div>
                </div>
    ...
    private updatePlaylistUI(): void {
        ...
        if (this.playlist.length === 0) {
            playlistItems.innerHTML = '<div style="text-align: center; color: #666; font-size: 10px; padding: 8px;">Playlist empty</div>';
            return;
        }
        ...
        this.playlist.forEach((track, index) => {
            ...
            const activeClass = isActive ? 'background: rgba(0,255,136,0.2); border-left: 3px solid #00ff88;' : '';
            const category = track.metadata?.category ? ` (${track.metadata.category})` : '';
            
            playlistHTML += `
                <div onclick="visual.loadTrack(${index})" 
                     style="padding: 4px 8px; cursor: pointer; font-size: 10px; border-radius: 3px; 
                            margin: 1px 0; transition: all 0.2s ease; ${activeClass}
                            ${!isActive ? 'hover: background: rgba(255,255,255,0.1);' : ''}"
                     onmouseover="this.style.background='rgba(255,255,255,0.1)'"
                     onmouseout="this.style.background='${isActive ? 'rgba(0,255,136,0.2)' : 'transparent'}'"
                     title="${track.name}${category}">
                    <div style="color: ${isActive ? '#00ff88' : '#fff'}; font-weight: ${isActive ? 'bold' : 'normal'};">
                        ${isActive ? '🎵 ' : ''}${track.name.length > 20 ? track.name.substring(0, 20) + '...' : track.name}
                    </div>
                    ${category ? `<div style="color: #888; font-size: 9px;">${category.substring(2, category.length-1)}</div>` : ''}
                </div>
            `;
        });
        
        playlistItems.innerHTML = playlistHTML;
        console.log(`🎵 Updated playlist UI with ${this.playlist.length} tracks`);
    }
    ...
    private resizeCanvases(): void {
        if (this.equalizerCanvas && this.particleCanvas && this.target) {
            // Get the actual visual container size
            const containerRect = this.target.getBoundingClientRect();
            
            // Account for controls at the top - reduced from 120px due to optimization
            const controlsHeight = 80; // Reduced from 120px since we optimized the layout
            const availableHeight = Math.max(250, containerRect.height - controlsHeight); // Increased min height
            const availableWidth = Math.max(300, containerRect.width - 20); // 20px for padding
            
            // Update canvas container size first
            const canvasContainer = this.target.querySelector('.canvas-container') as HTMLElement;
            if (canvasContainer) {
                canvasContainer.style.height = `${availableHeight}px`;
                canvasContainer.style.width = `${availableWidth}px`;
            }
            
            // Now get the updated rect for canvas sizing
            const rect = this.equalizerCanvas.getBoundingClientRect();
            
            // Set actual canvas size with proper pixel ratio
            ...
            // Update WebGL canvas size too
            if (this.webglCanvas) {
                this.webglCanvas.width = rect.width * window.devicePixelRatio;
                this.webglCanvas.height = rect.height * window.devicePixelRatio;
            }
            
            // Scale contexts to match device pixel ratio (only do this once per resize)
            if (this.canvasContext) {
                this.canvasContext.resetTransform(); // Reset before scaling
                this.canvasContext.scale(window.devicePixelRatio, window.devicePixelRatio);
            }
            if (this.particleContext) {
                this.particleContext.resetTransform(); // Reset before scaling
                this.particleContext.scale(window.devicePixelRatio, window.devicePixelRatio);
            }
            
            console.log(`📏 Canvas resized - Container: ${containerRect.width}x${containerRect.height}, Available: ${availableWidth}x${availableHeight}, Canvas: ${rect.width}x${rect.height}`);
        }
    }
    ...
    private createBeatParticles(): void {
        ...
        for (let i = 0; i < 8; i++) { // Increased from 5 to 8
            this.particles.push({
                ...
                vy: -Math.random() * 15 - 5, // Much higher initial velocity (-5 to -20)
                size: Math.random() * 8 + 4, // Larger particles (4-12 instead of 2-8)
                color: theme?.particleColors[Math.floor(Math.random() * theme.particleColors.length)] || '#00ff88',
                life: 2.5, // Longer life (2.5 instead of 1)
                maxLife: 2.5,
                type: 'bass'
            });
        }
    ...
    private updateParticles(): void {
        ...
        this.particles = this.particles.filter(particle => {
            ...
            // Age particle (slower aging for longer visibility)
            particle.life -= 0.005; // Reduced from 0.01 to 0.005
    ...
    private generateAmbientParticles(rect: DOMRect): void {
        ...
        if (Math.random() < avgFreq / 255 * 0.2) { // Increased spawn rate from 0.15 to 0.2
            const theme = this.themes.get(this.currentTheme);
            ...
            const newParticle: Particle = {
                x: spawnX,
                y: spawnY,
                vx: (Math.random() - 0.5) * 6, // Increased horizontal movement
                vy: Math.random() * Math.abs(maxVelocityY) + maxVelocityY / 2, // Adjusted velocity based on mode
                size: Math.random() * 6 + 3, // Larger particles (3-9 instead of 2-6)
                color: theme?.particleColors[Math.floor(Math.random() * theme.particleColors.length)] || '#00ff88',
                life: Math.random() * 3 + 2, // Longer life (2-5 instead of 1-3)
